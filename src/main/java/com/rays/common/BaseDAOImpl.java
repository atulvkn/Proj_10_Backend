...
public abstract class BaseDAOImpl<T extends BaseDTO> implements BaseDAOInt<T> {
	...
	public T findByUniqueKey(String attribute, Object val, UserContext userContext) {
		...
		System.out.println("Inside BaseDAOImpl in FINDBYUNIQUEKEY()---method"  +attribute+" "+val+ " "+userContext);

		Class<T> dtoClass = getDTOClass();
		System.out.println("Inside BaseDAOImpl in FINDBYUNIQUEKEY()---method DTO CLASS GET" +dtoClass);
		...
		System.out.println("AFTER PREDICATE VALUE GOT FROM DB "+val);

		if (userContext != null && !isZeroNumber(userContext.getOrgId())) {
			System.out.println("INSIDE IF(userContext != null && !isZeroNumber(userContext.getOrgId()))");
		...
		if (list.size() > 0) {
			dto = list.get(0);
			System.out.println(dto.getId()+" "+dto.getOrgName()+" "+dto.getOrgId());
		}
	...
	protected TypedQuery<T> createCriteria(T dto, UserContext userContext) {
		...
		System.out.println(" isgroup skipped");
